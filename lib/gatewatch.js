...
var GateWatch = function GateWatch(login, passwd, url) {
    this.config = {};
    this.config.login = login;
    this.config.passwd = passwd;
    this.config.url = url || 'https://www.gatewatch.de/api';
};
...
GateWatch.prototype.getEvent = function (id, callback) {
    var opt = {'request_type':'getEvent', 'request_value':util.format('{"eventId": %d}', id)};
    this.makeRequest(opt, callback);
};


GateWatch.prototype.getTickets = function (id, callback) {
    var opt = {'request_type':'getTickets', 'request_value':util.format('{"eventId": %d}', id)};
    this.makeRequest(opt, callback);
};
